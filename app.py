...
def _serialize_activity_logs(logs):
    ...
    payload = []
    for log in logs:
        customer = customer_lookup.get(log.customer_id)
        payload.append(
            {
                "id": log.id,
...
def admin():
    ...
    activity_payload = _serialize_activity_logs(activity_logs)
    ...
    return render_template(
        ...
        activity_logs=activity_payload,
        next_url=next_url,
    )
...
def admin_activity(customer_id: int):
    ...
    payload = _serialize_activity_logs(logs)

    return jsonify({"success": True, "logs": payload})


@app.route('/admin/activity-feed')
